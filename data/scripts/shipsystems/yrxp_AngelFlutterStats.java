...
//            		ship, null, "yrxp_lrbm_10k", location, angle, null);
                ...
                for (ShipEngineAPI eng : ship.getEngineController().getShipEngines()) {
                    ...
                    if ((eng.isActive() || eng.isSystemActivated()) && (level > 0f)) {
                        ...
                        Global.getCombatEngine().spawnExplosion(eng.getLocation(), ZERO, bigBoostColor,
                                BOOST_MULT * 4f * boostScale * eng.getEngineSlot().getWidth(), duration);
                        Global.getCombatEngine().spawnExplosion(eng.getLocation(), ZERO, boostColor,
                                BOOST_MULT * 2f * boostScale * eng.getEngineSlot().getWidth(), 0.15f);
                    }
                }
...
//        WeaponAPI weaponRef = ship.getAllWeapons().get(MathUtils.getRandomNumberInRange(0, ship.getAllWeapons().size()-1));
    ...
    private void createFlareFlares(ShipAPI ship, float currentSideBaseAngle, float currentAngle) {
        ...
        MissileAPI newMissile = (MissileAPI) Global.getCombatEngine().spawnProjectile(
        		ship, null, "yrxp_angel_flare_short", location, angle, null);
        newMissile.setFromMissile(true);
    }
    ...
    private void createFlareFlaresLong(ShipAPI ship, float currentSideBaseAngle, float currentAngle) {
        ...
        MissileAPI newMissile = (MissileAPI) Global.getCombatEngine().spawnProjectile(
        		ship, null, "yrxp_angel_flare_long", location, angle, null);
        newMissile.setFromMissile(true);
    }
    ...
    private void launchMissile(ShipAPI ship, float currentRadius,  float currentSideBaseAngle, float currentAngle) {
        ...
        MissileAPI newMissile = (MissileAPI) Global.getCombatEngine().spawnProjectile(
        		ship, null, weaponID, location, angle, null);
        newMissile.setFromMissile(true);
...
//            ship.addAfterimage(afterimageColor, renderItem.getKey().x, renderItem.getKey().y, -ship.getVelocity().x, -ship.getVelocity().y,
//            		renderItem.getValue(), 0f, 0.1f, 0.5f, true, false, false);
//      	}
        ...
        ship.addAfterimage(afterimageColor, randLoc.x, randLoc.y, -ship.getVelocity().x, -ship.getVelocity().y,
        		randRange, 0.0f, 0.2f, 1.0f, true, false, false);
